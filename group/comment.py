...
def comment_topic(topic_url, comment_dict):
    ...
    doubanutil.logger.info("in func comment_topic(), " + str(comment_dict) + "| status_code: " + str(r.status_code)  +  " , the content :  "  +  str(r.text))
    return r

def  comment_post(url,_header,_data):
	r = requests.post(url,  headers = _header, data = _data)
        doubanutil.logger.info("in func comment_topic(), " + str(_data) +   ", header : " + str(_header) +  "| status_code: " + str(r.status_code)  +  " , the content :  "  +  str(r.text))
	return  r
